  ...
  try {
    ...
    page.on("console", async (msg) => {
      ...
      if (
        ...
        const cooldownUntil = Date.now() + autoRead429CooldownMs;
        console.warn(
          `[auto-read] 429 detected for ${maskUsername(
            username
          )}, pausing reads until ${new Date(cooldownUntil).toISOString()}`,
        );
        //由于油候脚本它这个时候可能会导航到新的网页,会导致直接执行代码报错,所以使用这个来在每个新网页加载之前来执行
        try {
          await page.evaluateOnNewDocument((cooldownUntil) => {
            localStorage.setItem("autoLikeEnabled", "false");
            localStorage.setItem(
              "autoReadCooldownUntil",
          ...
          try {
            if (!page.isClosed || !page.isClosed()) {
              await page.evaluate((cooldownUntil) => {
                localStorage.setItem("autoLikeEnabled", "false");
                localStorage.setItem(
                  "autoReadCooldownUntil",
                  String(cooldownUntil),
